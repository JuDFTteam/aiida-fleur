...
def test_extract_structure_info(clear_database_aiida_fleur, generate_structure,
    ...
    calc_function.add_incoming(structure_bulk, link_type=LinkType.INPUT_CALC,
    ...
    # create a group
    group = create_group(name='test_group', nodes=pks[:2],
                         description='test_description')

    result = extract_structure_info(keys=['uuid', 'formula', 'pk', 'symmetry', 'pbc', 'volume',
                                          'total_energy', 'child_nodes', 'natoms', 'group',
                                          'label', 'description', 'cif_file',
                                          'cif_number', 'cif_uuid', 'cif_ref', 'calcfunctions',
                                          'band', 'dos', 'eos', 'init_cls', 'corehole', 'primitive',
                                          'cell', 'scf'])
