...
class TestSetInpchanges:
    ...
    def test_set_inpchanges(self, inpxml_etree, name, path):
        ...
        skip_paths = ['atomSpecies', 'atomGroups', 'bzIntegration', 'kPointCount', 'bulkLattice', 'bravaisMatrix',
                      'energyParameterLimits', 'densityOfStates', 'a1']
...
def test_inpxml_to_dict(inpxml_etree):
    ...
    with open('../tests/files/parsed_inpxml.txt', "r") as f:
        correct = f.read()

    assert str(inpxml_dict) == correct
...
class TestShiftValue:
    ...
    def test_shift_value(self, inpxml_etree, attr_name):
        ...
        if not result_before:
            if attr_name in ['nx', 'ny', 'nz', 'scale', 'isec1', 'ndir', 'layers', 'lpr', 'qfix']:
                pytest.skip('This attribute is not tested for FePt/inp.xml')
